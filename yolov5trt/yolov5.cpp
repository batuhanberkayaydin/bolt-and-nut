...
int main(int argc, char** argv) {
    ...
    float nmsThresh = confParams["nmsThresh"];
    ...
    tracker = std::shared_ptr<TRACK>(new TRACK(trackingMaxAge));
    int frame_id = 0;
    std::vector<std::vector<int>> boxes_;
    std::string className[2] = {"bolt", "nut"};
    cv::Mat img;
    cv::VideoCapture capture(img_dir);
    capture >> img;
    cv::VideoWriter writeWideo;
    if (isRecordingEnable) {

        if (isH264){
            writeWideo.open("result.h264", cv::VideoWriter::fourcc('H','2','6','4'), 30, cv::Size(img.rows,img.cols),true);

        }
        else {
            writeWideo.open("result.avi", cv::VideoWriter::fourcc('M','J','P','G'), 30, cv::Size(img.rows,img.cols),true);
    ...
    while (true)
    {
        ...
        nms(res_pre, &prob[0], confThresh, nmsThresh);
        ...
        for (size_t j = 0; j < res_pre.size(); j++) {            
            ...
            cv::Rect r = get_rect(img, res_pre[j].bbox);
            cv::rectangle(img, r, cv::Scalar(0,255,0), 2);
            cv::putText(img,
                className[(int)res_pre[j].class_id],
                cv::Point2f(r.tl().x, r.tl().y),
                cv::FONT_HERSHEY_PLAIN,
                2,
                cv::Scalar(0,0,255),2);

            struct Bbox bbox1;
            bbox1.score = res_pre[j].conf;
            bbox1.x1 = r.tl().x;
            bbox1.y1 = r.tl().y;
            bbox1.w = r.br().x - r.tl().x;
            bbox1.h = r.br().y - r.tl().y;
            bbox1.className = className[(int)res_pre[j].class_id];
            bboxes.push_back(bbox1);
            for (std::vector<struct Bbox>::iterator it = bboxes.begin(); it != bboxes.end(); it++) {
        ...
        for (TrackingBox it : tracking_results) {
            ...
            cv::putText(img,
                    std::to_string(it.id),
                    cv::Point2f(it.box.x -5 , it.box.y - 25),
                    cv::FONT_HERSHEY_PLAIN,
                    2,
                    cv::Scalar(0,0,255),2);
